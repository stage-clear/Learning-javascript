...
- 関数でなくオブジェクトを返すという点を除けば, 即時関数にきわめてよく似ている.

```javascript
// javaScript Design Patters [26]
...
console.log( basketModule.getTital() );
```
...
メソッド呼び出しやリクエスト, あるいはオペレーションを, 1つのオブジェクトにカプセル化し, メソッド呼び出しのパラメータ化とmそっど呼び出しを渡すことのどちらも自由に実行できるようにすることです.
